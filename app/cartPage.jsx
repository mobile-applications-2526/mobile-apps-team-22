...
const CartPage = () => {
  ...
  const [pickupTime, setPickupTime] = useState("ASAP");
  const [isTimeModalVisible, setTimeModalVisible] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false); // Loading state for checkout button
      ...
      Alert.alert(
        ...
        [
          ...
          { 
            text: "Sign In / Register", 
            onPress: () => {
              // Redirect to Login, passing return param
              router.push({
                pathname: '/login',
                params: { returnTo: 'cartPage' }  
              });
